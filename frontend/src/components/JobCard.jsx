    ...
    <Link to={`/jobs/${job._id}`} className="block">
      <div className="rounded-lg bg-gray-800 p-6 shadow-lg transition-shadow duration-300 hover:shadow-green-500/20 h-full flex flex-col">
         ...
         <img
            src={
              job.companyInfo?.logoURL 
                ? `http://localhost:4000/${job.companyInfo.logoURL}` 
                : job.companyId?.logoURL 
                  ? `http://localhost:4000/${job.companyId.logoURL}` 
          ...
          <div>
            ...
            <p className="text-gray-400">{job.companyId?.name || job.companyInfo.name}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between text-sm text-gray-400 mt-auto">
          <span>{job.companyId?.location?.split(',').pop().trim() || job.companyInfo.location.split(',').pop().trim() }</span> {/* Display only city/province */}
          <span>{job.level}</span>
        </div>
      </div>
    </Link>
